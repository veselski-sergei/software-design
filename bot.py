...
chat_allow1 = 342212677  # Егор # поменять на массив айдишников здесь и везде где проверка идет по id
chat_allow2 = 257432313  # Миша
# 392001967  Серега
...
async def on_chat_message(msg):
    ...
    if (chat_id == chat_allow1 or chat_id == chat_allow2):
            ...
            if (id_write_critical_temper == 1):
                ...
                if (command.isdigit()):
                    ...
                    await bot.sendMessage(chat_id,
                                          str("Температурный минимум установлен в %s градусов. Ниже этой температуры будут приходить алерты") % command,
                                          reply_markup=markup)
                    id_write_critical_temper = 0
                else:
                    markup = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text='главное меню')]])
                    await bot.sendMessage(chat_id,
                                          str("%s - это не целое число. При необходимости пройдите настройку заново. Значение не установлено!") % command,
                                          reply_markup=markup)
                    id_write_critical_temper = 0
            else:
        ...
        await bot.sendMessage(chat_id, 'Вы не имеете доступа к этому боту! Обратитесь к владельцу за разрешением.',
                              reply_markup=markup_protect)
        return


# эта функция отвечает за "полупрозрачные кнопки"
